import Pagina1 from "../pages/pagina1";
import Pagina2 from "../pages/pagina2";
import Pagina3 from "../pages/pagina3";
import Pagina4 from "../pages/pagina4";
import Pagina5 from "../pages/pagina5";
import Pagina6 from "../pages/pagina6";
import Pagina7 from "../pages/pagina7";
import Pagina8 from "../pages/pagina8";
import Pagina9 from "../pages/pagina9";
import Pagina10 from "../pages/pagina10";
import Pagina11 from "../pages/pagina11";
import Pagina12 from "../pages/pagina12";
import Pagina13 from "../pages/pagina13";
import Pagina14 from "../pages/pagina14";
import Pagina15 from "../pages/pagina15";
import Pagina16 from "../pages/pagina16";
import Pagina17 from "../pages/pagina17";
import Pagina18 from "../pages/pagina18";
import Pagina19 from "../pages/pagina19";
import Pagina20 from "../pages/pagina20";
import Pagina21 from "../pages/pagina21";
import Pagina22 from "../pages/pagina22";
import Pagina23 from "../pages/pagina23";
import Pagina24 from "../pages/pagina24";
import Pagina25 from "../pages/pagina25";
import Pagina26 from "../pages/pagina26";
import Pagina27 from "../pages/pagina27";
import Pagina28 from "../pages/pagina28";
import Pagina29 from "../pages/pagina29";
import Pagina30 from "../pages/pagina30";
import Pagina31 from "../pages/pagina31";
import Pagina32 from "../pages/pagina32";
import Pagina33 from "../pages/pagina33";
import Pagina34 from "../pages/pagina34";
import Pagina35 from "../pages/pagina35";
import Pagina36 from "../pages/pagina36";
import Pagina37 from "../pages/pagina37";
import Pagina38 from "../pages/pagina38";
import Pagina39 from "../pages/pagina39";
import Pagina40 from "../pages/pagina40";
import Pagina41 from "../pages/pagina41";
import Pagina42 from "../pages/pagina42";
import Pagina43 from "../pages/pagina43";
import Pagina44 from "../pages/pagina44";
import Pagina45 from "../pages/pagina45";
import Pagina46 from "../pages/pagina46";
import Pagina47 from "../pages/pagina47";
import Pagina48 from "../pages/pagina48";
import Pagina49 from "../pages/pagina49";
import Pagina50 from "../pages/pagina50";
import Pagina51 from "../pages/pagina51";
import Pagina52 from "../pages/pagina52";
import Pagina53 from "../pages/pagina53";
import Pagina54 from "../pages/pagina54";
import Pagina55 from "../pages/pagina55";
import Pagina56 from "../pages/pagina56";
import Pagina57 from "../pages/pagina57";
import Pagina58 from "../pages/pagina58";
import Pagina59 from "../pages/pagina59";
import Pagina60 from "../pages/pagina60";
import Pagina61 from "../pages/pagina61";
import Pagina62 from "../pages/pagina62";
import Pagina63 from "../pages/pagina63";
import Pagina64 from "../pages/pagina64";
import Pagina65 from "../pages/pagina65";
import Pagina66 from "../pages/pagina66";
import Pagina67 from "../pages/pagina67";
import Pagina68 from "../pages/pagina68";
import Pagina69 from "../pages/pagina69";
import Pagina70 from "../pages/pagina70";
import Pagina71 from "../pages/pagina71";
import Pagina72 from "../pages/pagina72";
import Pagina73 from "../pages/pagina73";
import Pagina74 from "../pages/pagina74";
import Pagina75 from "../pages/pagina75";
import Pagina76 from "../pages/pagina76";
import Pagina77 from "../pages/pagina77";
import Pagina78 from "../pages/pagina78";
import Pagina79 from "../pages/pagina79";
import Pagina80 from "../pages/pagina80";
import Pagina81 from "../pages/pagina81";
import Pagina82 from "../pages/pagina82";
import Pagina83 from "../pages/pagina83";
import Pagina84 from "../pages/pagina84";
import Pagina85 from "../pages/pagina85";
import Pagina86 from "../pages/pagina86";
import Pagina87 from "../pages/pagina87";
import Pagina88 from "../pages/pagina88";
import Pagina89 from "../pages/pagina89";
import Pagina90 from "../pages/pagina90";
import Pagina91 from "../pages/pagina91";
import Pagina92 from "../pages/pagina92";
import Pagina93 from "../pages/pagina93";
import Pagina94 from "../pages/pagina94";
import Pagina95 from "../pages/pagina95";
import Pagina96 from "../pages/pagina96";
import Pagina97 from "../pages/pagina97";
import Pagina98 from "../pages/pagina98";
import Pagina99 from "../pages/pagina99";
import Pagina100 from "../pages/pagina100";
import Pagina101 from "../pages/pagina101";
import Pagina102 from "../pages/pagina102";
import Pagina103 from "../pages/pagina103";
import Pagina104 from "../pages/pagina104";
import Pagina105 from "../pages/pagina105";
import Pagina106 from "../pages/pagina106";
import Pagina107 from "../pages/pagina107";
import Pagina108 from "../pages/pagina108";
import Pagina109 from "../pages/pagina109";
import Pagina110 from "../pages/pagina110";
import Pagina111 from "../pages/pagina111";
import Pagina112 from "../pages/pagina112";
import Pagina113 from "../pages/pagina113";
import Pagina114 from "../pages/pagina114";
import Pagina115 from "../pages/pagina115";
import Pagina116 from "../pages/pagina116";
import Pagina117 from "../pages/pagina117";
import Pagina118 from "../pages/pagina118";
import Pagina119 from "../pages/pagina119";
import Pagina120 from "../pages/pagina120";
import Pagina121 from "../pages/pagina121";
import Pagina122 from "../pages/pagina122";
import Pagina123 from "../pages/pagina123";
import Pagina124 from "../pages/pagina124";
import Pagina125 from "../pages/pagina125";
import Pagina126 from "../pages/pagina126";
import Pagina127 from "../pages/pagina127";
import Pagina128 from "../pages/pagina128";
import Pagina129 from "../pages/pagina129";
import Pagina130 from "../pages/pagina130";


const routes = [
  { path: "/", Component: Pagina1 },
  { path: "/pagina2", Component: Pagina2 },
  { path: "/pagina3", Component: Pagina3 },
  { path: "/pagina4", Component: Pagina4 },
  { path: "/pagina5", Component: Pagina5 },
  { path: "/pagina6", Component: Pagina6 },
  { path: "/pagina7", Component: Pagina7 },
  { path: "/pagina8", Component: Pagina8 },
  { path: "/pagina9", Component: Pagina9 },
  { path: "/pagina10", Component: Pagina10 },
  { path: "/pagina11", Component: Pagina11 },
  { path: "/pagina12", Component: Pagina12 },
  { path: "/pagina13", Component: Pagina13 },
  { path: "/pagina14", Component: Pagina14 },
  { path: "/pagina15", Component: Pagina15 },
  { path: "/pagina16", Component: Pagina16 },
  { path: "/pagina17", Component: Pagina17 },
  { path: "/pagina18", Component: Pagina18 },
  { path: "/pagina19", Component: Pagina19 },
  { path: "/pagina20", Component: Pagina20 },
  { path: "/pagina21", Component: Pagina21 },
  { path: "/pagina22", Component: Pagina22 },
  { path: "/pagina23", Component: Pagina23 },
  { path: "/pagina24", Component: Pagina24 },
  { path: "/pagina25", Component: Pagina25 },
  { path: "/pagina26", Component: Pagina26 },
  { path: "/pagina27", Component: Pagina27 },
  { path: "/pagina28", Component: Pagina28 },
  { path: "/pagina29", Component: Pagina29 },
  { path: "/pagina30", Component: Pagina30 },
  { path: "/pagina31", Component: Pagina31 },
  { path: "/pagina32", Component: Pagina32 },
  { path: "/pagina33", Component: Pagina33 },
  { path: "/pagina34", Component: Pagina34 },
  { path: "/pagina35", Component: Pagina35 },
  { path: "/pagina36", Component: Pagina36 },
  { path: "/pagina37", Component: Pagina37 },
  { path: "/pagina38", Component: Pagina38 },
  { path: "/pagina39", Component: Pagina39 },
  { path: "/pagina40", Component: Pagina40 },
  { path: "/pagina41", Component: Pagina41 },
  { path: "/pagina42", Component: Pagina42 },
  { path: "/pagina43", Component: Pagina43 },
  { path: "/pagina44", Component: Pagina44 },
  { path: "/pagina45", Component: Pagina45 },
  { path: "/pagina46", Component: Pagina46 },
  { path: "/pagina47", Component: Pagina47 },
  { path: "/pagina48", Component: Pagina48 },
  { path: "/pagina49", Component: Pagina49 },
  { path: "/pagina50", Component: Pagina50 },
  { path: "/pagina51", Component: Pagina51 },
  { path: "/pagina52", Component: Pagina52 },
  { path: "/pagina53", Component: Pagina53 },
  { path: "/pagina54", Component: Pagina54 },
  { path: "/pagina55", Component: Pagina55 },
  { path: "/pagina56", Component: Pagina56 },
  { path: "/pagina57", Component: Pagina57 },
  { path: "/pagina58", Component: Pagina58 },
  { path: "/pagina59", Component: Pagina59 },
  { path: "/pagina60", Component: Pagina60 },
  { path: "/pagina61", Component: Pagina61 },
  { path: "/pagina62", Component: Pagina62 },
  { path: "/pagina63", Component: Pagina63 },
  { path: "/pagina64", Component: Pagina64 },
  { path: "/pagina65", Component: Pagina65 },
  { path: "/pagina66", Component: Pagina66 },
  { path: "/pagina67", Component: Pagina67 },
  { path: "/pagina68", Component: Pagina68 },
  { path: "/pagina69", Component: Pagina69 },
  { path: "/pagina70", Component: Pagina70 },
  { path: "/pagina71", Component: Pagina71 },
  { path: "/pagina72", Component: Pagina72 },
  { path: "/pagina73", Component: Pagina73 },
  { path: "/pagina74", Component: Pagina74 },
  { path: "/pagina75", Component: Pagina75 },
  { path: "/pagina76", Component: Pagina76 },
  { path: "/pagina77", Component: Pagina77 },
  { path: "/pagina78", Component: Pagina78 },
  { path: "/pagina79", Component: Pagina79 },
  { path: "/pagina80", Component: Pagina80 },
  { path: "/pagina81", Component: Pagina81 },
  { path: "/pagina82", Component: Pagina82 },
  { path: "/pagina83", Component: Pagina83 },
  { path: "/pagina84", Component: Pagina84 },
  { path: "/pagina85", Component: Pagina85 },
  { path: "/pagina86", Component: Pagina86 },
  { path: "/pagina87", Component: Pagina87 },
  { path: "/pagina88", Component: Pagina88 },
  { path: "/pagina89", Component: Pagina89 },
  { path: "/pagina90", Component: Pagina90 },
  { path: "/pagina91", Component: Pagina91 },
  { path: "/pagina92", Component: Pagina92 },
  { path: "/pagina93", Component: Pagina93 },
  { path: "/pagina94", Component: Pagina94 },
  { path: "/pagina95", Component: Pagina95 },
  { path: "/pagina96", Component: Pagina96 },
  { path: "/pagina97", Component: Pagina97 },
  { path: "/pagina98", Component: Pagina98 },
  { path: "/pagina99", Component: Pagina99 },
  { path: "/pagina100", Component: Pagina100 },
  { path: "/pagina101", Component: Pagina101 },
  { path: "/pagina102", Component: Pagina102 },
  { path: "/pagina103", Component: Pagina103 },
  { path: "/pagina104", Component: Pagina104 },
  { path: "/pagina105", Component: Pagina105 },
  { path: "/pagina106", Component: Pagina106 },
  { path: "/pagina107", Component: Pagina107 },
  { path: "/pagina108", Component: Pagina108 },
  { path: "/pagina109", Component: Pagina109 },
  { path: "/pagina110", Component: Pagina110 },
  { path: "/pagina111", Component: Pagina111 },
  { path: "/pagina112", Component: Pagina112 },
  { path: "/pagina113", Component: Pagina113 },
  { path: "/pagina114", Component: Pagina114 },
  { path: "/pagina115", Component: Pagina115 },
  { path: "/pagina116", Component: Pagina116 },
  { path: "/pagina117", Component: Pagina117 },
  { path: "/pagina118", Component: Pagina118 },
  { path: "/pagina119", Component: Pagina119 },
  { path: "/pagina120", Component: Pagina120 },
  { path: "/pagina121", Component: Pagina121 },
  { path: "/pagina122", Component: Pagina122 },
  { path: "/pagina123", Component: Pagina123 },
  { path: "/pagina124", Component: Pagina124 },
  { path: "/pagina125", Component: Pagina125 },
  { path: "/pagina126", Component: Pagina126 },
  { path: "/pagina127", Component: Pagina127 },
  { path: "/pagina128", Component: Pagina128 },
  { path: "/pagina129", Component: Pagina129 },
  { path: "/pagina130", Component: Pagina130 }  
];

export default routes;
